  ...
  setupLogging() {
    ...
    prefix.apply(log, {
      format(level, name, timestamp) {
        const color = colors[level.toUpperCase()] || colors.BLUE_LIGHT;
      ...
      for (let i = 0; i < data.length; i++) {
        ...
        if (userInfo.is_exist) {
          ...
          if (collectInfo) {
            ...
            log.info(
              `Available Snitch: ${collectInfo.available_snitch} | Available USDT: ${collectInfo.usdt.toFixed(6)}`
            );
            ...
            if (collectInfo.available_snitch > 0) {
              ...
              log.info(
                `Collecting ${(randomPercentage * 100).toFixed(2)}% of ${collectInfo.available_snitch} available snitch`
              );
